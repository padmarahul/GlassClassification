# -*- coding: utf-8 -*-
"""CSCE_5310_Final_Project.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1nqiGewhyC4K9XSucniE7a6FMXUwbeBSQ
"""
...
## Immideately we found out that the Type feature
## is mislabeled where a class is missing.

print(data.describe())

## Handling the missied class by renaming the class values
...
## We use Q1 and Q3 to find the interquartile range and then create a step
## size by multiplying it with 1.5. We then use then use this as a refference
## to find out outliers. The formula is presented in the code section
...
## The function has found out 14 outliers, so we dropped them.
...
## It also provides a distributionplot with a seperate distributiion for each
## class seperately. We can clearly see that the distribution of different
## classes are intersecting, meaning classes are intersecting.

sns.pairplot(data, hue="Type")

## Trying to understand the relation between each pair of features numrically.
## The heat map shows the correlation between each feature. Some features such
## as RI and Ca are highly correlated with many other features.

plt.figure(figsize=(8,8))
sns.heatmap(data.corr(), annot=True)

## Checking how the data is distubuted in each feature seperately.
## we have also used the Kernel Density Estimation to produce continuous plot
## for the distriobution.
...
## Implementing box plot to visually see the quartiles and outliers
## present in each class seperatley. This helped us to write the
## outlier detection algorithm presented in the early section.

plt.figure(figsize=(15,10))
plt.title("Box plot for each feature")
sns.boxplot(data.iloc[:, :-1], orient='h')

## Checking the count of classes. This shows that the dataset is
## imbalanced with very few samples in classes 2, 3, 4 and 5.
...
## Splitting the dataset into training and testing set using model selection
## technique. Later we have standerdized the data using the standard scaler.
...
## Initializing the required models and cvariables for the training and
## validation proes. 7 different model has been tested with one model having
## differernt variations (Random Forest).
models = {
...
## Comparing the performance of each model using the cross validation tecnique
## We have implemented the 10 fold cross validation to check the median performance
## of each model. The test has proved Gradient Boosting Algorithm is performing
## better since the median performance is good along with high score.
...
## It has performed very well on the training set and the results
## are presented below.
...
## The model performance on the test set. It has a accuracy score of 85%
## which is a good score considering the size of the dataset and imbalanced
## it is.

print(classification_report(Y[1], models['Gradient Boosting'].predict(X[1])))

## Confusion Matrix for test set. It has a few misclassifications in it.
